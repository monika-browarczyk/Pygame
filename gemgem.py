...
def runGame():
    ...
    while True: # main game loop
        clickedSpace = None
        timeLeft = max(0, int(COUNTDOWN_TIME - (time.time() - startTime)))
        if timeLeft == 0:
            gameIsOver = True
...
def drawTimer(timeLeft):
    ...
    timerImg = BASICFONT.render(timerText, 1, SCORECOLOR)
    timerRect = timerImg.get_rect()
    timerRect.topright = (WINDOWWIDTH - 250, 10)
    DISPLAYSURF.blit(timerImg, timerRect)
