...
def main():
    ...
    global COMPUTERWINNERIMG, WINNERRECT, TIEWINNERIMG, BASICFONT
    ...
    REDTOKENIMG = pygame.image.load('assets/4row_red.png')
    REDTOKENIMG = pygame.transform.smoothscale(REDTOKENIMG, (SPACESIZE, SPACESIZE))
    BLACKTOKENIMG = pygame.image.load('assets/4row_black.png')
    BLACKTOKENIMG = pygame.transform.smoothscale(BLACKTOKENIMG, (SPACESIZE, SPACESIZE))
    BOARDIMG = pygame.image.load('assets/4row_board.png')
    BOARDIMG = pygame.transform.smoothscale(BOARDIMG, (SPACESIZE, SPACESIZE))

    HUMANWINNERIMG = pygame.image.load('assets/4row_humanwinner.png')
    COMPUTERWINNERIMG = pygame.image.load('assets/4row_computerwinner.png')
    TIEWINNERIMG = pygame.image.load('assets/4row_tie.png')
    WINNERRECT = HUMANWINNERIMG.get_rect()
    WINNERRECT.center = (int(WINDOWWIDTH / 2), int(WINDOWHEIGHT / 2))

    ARROWIMG = pygame.image.load('assets/4row_arrow.png')
...
def drawBoard(board, extraToken=None, turn=None):
    ...
    if text:
        turnSurf = BASICFONT.render(text, True, WHITE)
        turnRect = turnSurf.get_rect()
        turnRect.center = (WINDOWWIDTH // 2, 30)
...
def getHumanMove(board, isFirstMove):
    ...
    while True:
        ...
        if tokenx != None and tokeny != None:
            drawBoard(board, {'x':tokenx - int(SPACESIZE / 2), 'y':tokeny - int(SPACESIZE / 2), 'color':RED}, turn=HUMAN)
        else:
            drawBoard(board, turn=HUMAN)
...
def animateDroppingToken(board, column, color):
    ...
    while True:
        ...
        if int((y - YMARGIN) / SPACESIZE) >= lowestEmptySpace:
            return
        drawBoard(board, {'x':x, 'y':y, 'color':color}, turn=HUMAN)
        pygame.display.update()
        FPSCLOCK.tick()
